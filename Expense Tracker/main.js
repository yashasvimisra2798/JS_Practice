...
function displayExp(details) {
  ...
  for (i = 0; i < details.length; i++) {
    expValue.innerHTML += `
    <div class="expValue" id="Rs.{details[i].id}">
      <div id="expTitleName" class="exp"><p>Rs.{details[i].name}</p></div>
      <div id="expValueAmount" class="exp"><p> <span>Rs. </span> Rs.{details[i].number}</p></div>
      <div id="edite_delete">
        <p>
          <button id="Rs.{details[i].id}" onclick="editExpDetails(Rs.{details[i].id})"> <img src="image/edit.svg" width="15" alt=""  /></button> 
          <button id="Rs.{details[i].id}" onclick="delExpenseDetails(Rs.{details[i].id})"><img src="image/trash.svg" width="15" alt="" /></button>
        </p>
      </div>
    </div>
  `;
